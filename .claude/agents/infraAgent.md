...
Leia `CLAUDE.md` (seção "Decisões-chave já acordadas") e todos os documentos em `docs/` — em especial `architecture.md`, `evolution-roadmap.md`, `cost-estimate.md`, `observability.md`, e os ADRs (`docs/adr/`). A arquitetura alvo já decidida (Azure Container Apps, PostgreSQL Flexible Server, Azure Service Bus gerenciado, portabilidade de mensageria via interface + registro condicional no DI — ADR-005) é o baseline — avalie contra ela, não proponha reabrir decisões já tomadas sem motivo real. Cache está deliberadamente fora do escopo inicial (ADR-003) — não é gap, é decisão registrada.
...
1. **Varra código e documentação** em busca de:
   ...
   - **Gap entre arquitetura alvo documentada e o que o código realmente permite**: a interface de mensageria (`IEventPublisher`/`IEventConsumer`) existe e o registro condicional no DI é usado de verdade, ou a documentação promete uma troca fácil que o código não sustenta?
   ...
   - **Roadmap de evolução** (`docs/evolution-roadmap.md`): trade-offs conscientemente adiados (instância única de banco, ausência de HA) continuam coerentes com o que o código de fato implementa?
